...
def getArguments():
    ...
    parser.add_argument('--gpu', type=int, default=0, help='0: CPU; 1: GPU')

    return parser.parse_args()


def get_index(vector_size, gpu=False):
...
def get_embeddings_as_array(w2v):
    res = np.ones((len(w2v.key_to_index), w2v.vector_size), dtype='float32')

    for i, w in enumerate(w2v.index_to_key):
        vec = w2v[w]
        res[i] = vec / np.linalg.norm(vec)

    return res


def get_nn(se, te, knn, gpu=False):
...
def get_csls(se, te, knn, csls_knn, gpu=False):
    assert knn <= csls_knn
...
def main(source_embeddings, binary, output, target_embeddings=None, knn=10,
         max_vocab=None, float_precision=4, method='nn', cslsknn=10, gpu=False):
    method = method.split('-')
    method = method[0]
    ...
    with open(output, 'w') as fout:
        for i, idxs in enumerate(indices):
            ...
            tmp = ['{}\t{}'.format(te.index_to_key[idxs[j]], format(distances[i][j], '.{}f'.format(float_precision))) for j in range(knn)]
